...
name= toString("inputfiles/gIzh_sparse_ind_ee");
    ...
    PCNN.read_sparsesyns_par("Exc_Exc", CExc_Exc, f_ind, f_indInG,f,gExc_Exc);
    fclose(f);
    fclose(f_indInG);
    fclose(f_ind);
    ...
    f_ind= fopen(name.c_str(),"rb");
    PCNN.read_sparsesyns_par("Exc_Inh", CExc_Inh, f_ind,f_indInG,f,gExc_Inh);
    fclose(f);
    fclose(f_indInG);
    fclose(f_ind);
    ...
    f_ind= fopen(name.c_str(),"rb");
    PCNN.read_sparsesyns_par("Inh_Exc", CInh_Exc, f_ind,f_indInG,f,gInh_Exc);
    fclose(f);
    fclose(f_indInG);
    fclose(f_ind);
    ...
    f_ind= fopen(name.c_str(),"rb");
    PCNN.read_sparsesyns_par("Inh_Inh", CInh_Inh, f_ind,f_indInG,f,gInh_Inh);
    fclose(f);
    fclose(f_indInG);
    fclose(f_ind);
#ifndef CPU_ONLY
...
*/
  ...
  auto *pExc = PCNN.model.findNeuronGroup("PExc");
  auto *pInh = PCNN.model.findNeuronGroup("PInh");
#ifndef CPU_ONLY
  int sampleBlkNo0 = ceil(float((pExc->getNumNeurons())/float(BlkSz)));
  int sampleBlkNo1 = ceil(float((pInh->getNumNeurons())/float(BlkSz)));
...
#ifndef CPU_ONLY
      CHECK_CUDA_ERRORS(cudaMalloc((void **)&devStates, pExc->getNumNeurons()*sizeof(curandState)));
      xorwow_setup(devStates, pExc->getNumNeurons(), 117); //setup the prng for the bigger network only

      generate_random_gpuInput_xorwow(devStates, d_I0PExc, pExc->getNumNeurons(), meanInpExc, (scalar) 0.0, sGrid0, sThreads);
      generate_random_gpuInput_xorwow(devStates, d_I0PInh, pInh->getNumNeurons(), meanInpInh, (scalar) 0.0, sGrid1, sThreads);
#endif
  }  
  ...
  if (which == GPU){ 
	  ...
	  for (int i= 0; i < glbSpkCntPInh[0]; i++) {
	      fprintf(osf2, "%f %d\n", t, pInh->getIDRange().first+glbSpkPInh[i]);
	  }
	  // report progress
