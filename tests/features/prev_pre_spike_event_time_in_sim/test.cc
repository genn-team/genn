//--------------------------------------------------------------------------
/*! \file prev_pre_spike_event_time_in_sim/test.cc
...
#include "prev_pre_spike_event_time_in_sim_CODE/definitions.h"
        ...
        while(t < 200.0f) {
            ...
            if(t > DT) {
                ...
                for(unsigned int i = 0; i < 10; i++) {
                    ...
                    const float delayedLastSpikeEventTime = (scalar)i + 11.0f + (10.0f * std::floor((t - 22.0f - (scalar)i) / 10.0f));
                 
                    // If, theoretically, spike would have arrived before delay it's impossible so time should be a very large negative number
                    if(delayedLastSpikeEventTime < 21.0f) {
                        ASSERT_LT(wsyn[i], -1.0E6);
                    }
                    else {
                        ASSERT_FLOAT_EQ(wsyn[i], delayedLastSpikeEventTime);
                    }
                }
            }
        }
...
TEST_F(SimTest, PrevPreSpikeEventTimeInSim)
{
    Simulate();
}
