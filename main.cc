...
void generateNeuronUpdateKernel(CodeStream &os, const NNmodel &model, const CodeGenerator::Base &codeGenerator)
{
    ...
    codeGenerator.genNeuronUpdateKernel(os, model,
        [](CodeStream &os, const CodeGenerator::Base &codeGenerator, const NNmodel &model)
        {
            // Generate code for each neuron group
            codeGenerator.genForEachNeuronGroup(os, model,
                [](CodeStream &os, const CodeGenerator::Base &codeGenerator, const NNmodel &model, const NeuronGroup &ng)
                {
                    if (ng.isVarQueueRequired() && ng.isDelayRequired()) {
                        os << "const unsigned int delaySlot = (" << codeGenerator.getVarPrefix() << "spkQuePtr" << ng.getName();
                        os << " + " << (ng.getNumDelaySlots() - 1);
                        os << ") % " << ng.getNumDelaySlots() << ";" << std::endl;
                    }

                    // Generate code for each neuron in group
                    codeGenerator.genForEachNeuron(os, model, ng,
                        [](CodeStream &os, const CodeGenerator::Base &codeGenerator, const NNmodel &model, const NeuronGroup &ng, const std::string &neuronID, const std::string &rngName)
                        {
