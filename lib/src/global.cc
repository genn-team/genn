...
namespace GENN_PREFERENCES {    
    ...
    VarLocation defaultVarLocation = VarLocation::HOST_DEVICE;  //!< What is the default behaviour for model state variables? Historically, everything was allocated on both host AND device and initialised on HOST.
    VarLocation defaultSparseConnectivityLocation = VarLocation::HOST_DEVICE;   //! What is the default behaviour for sparse synaptic connectivity? Historically, everything was allocated on both the host AND device and initialised on HOST
