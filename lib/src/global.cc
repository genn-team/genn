...
namespace GENN_PREFERENCES {    
    ...
    bool learnPostBeforePre = false;    //!< Should postsynaptic learning kernel be run before rather than after presynaptic update kernel
...
unsigned int initSparseBlkSz; //!< Global variable containing the GPU block size for the sparse initialization kernel
