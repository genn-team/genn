    ...
    )
{
	...
	for (int j = 0; j < model.neuronGrpN; j++) {
		...
		os << "d_glbSpkCntEvnt" << model.neuronName[j] << "[d_spkEvntQuePtr";
	...
	if (nt != POISSONNEURON) {
	    ...
	    for (int j = 0; j < model.inSyn[i].size(); j++) {
		os << model.ftype << " linSyn" << j << " = dd_inSyn" << model.synapseName[model.inSyn[i][j]] << "[" << localID << "];" << ENDL;
	    }
		...
		for (int j = 0; j < model.inSyn[i].size(); j++) {
			...
			os << " = dd_" <<  psm.varNames[k] << model.synapseName[synPopID] << "[";
			os << localID << "];" << ENDL;
		      }
		    }
		    os << "Isyn += ";
	...
	for (int j = 0; j < model.inSyn[i].size(); j++) {
	    ...
	    os << psCode << ENDL;
	    os << "dd_inSyn"  << model.synapseName[model.inSyn[i][j]] << "[" << localID << "] = linSyn"<< j << ";" << ENDL;
...
{
    cerr << "entering genSynapseKernel ..." << endl;
    string name, 
	s, 
   ...
   //case-dependent variables
    // TODO: worry about when linSyn is needed
    os << model.ftype << " linSyn;" << ENDL;
    // we need ipost, npost if there are any SPARSE connections
   ...
   for (int i = 0; i < model.synapseGrpN; i++) {
	...
	if (model.usesSpikeEvents[i] == TRUE) {	
	    generate_process_presynaptic_events_code(os, model, src, trg, i, localID, Epre, inSynNo, tS("Evnt"));
	}
	// generate the code for processing true spike events
	if (model.usesTrueSpikes[i] == TRUE) {
	    generate_process_presynaptic_events_code(os, model, src, trg, i, localID, Epre, inSynNo, tS(""));
	}       
