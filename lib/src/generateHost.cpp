...
void genHostCode(ostream &mos)
{
  ...
  hppFile = path + tS("/") + model->name + tS("_CODE_HOST/host.hpp");
  osh.open(hppFile.c_str());
  writeHeader(osh);
  osh << endl;
  osh << "//-------------------------------------------------------------------------" << endl;
  osh << "/*! \\file host.hpp" << endl << endl;
  ...
  //---------------------------
  // open and setup control.cpp

  cppFile = path + tS("/") + model->name + tS("_CODE_HOST/control.cpp");
  os.open(cppFile.c_str());
  writeHeader(os);
  os << endl;
  os << "//-------------------------------------------------------------------------" << endl;
  os << "/*! \\file control.cpp" << endl << endl;
  ...
  //-------------------------
  // add control.cpp includes

  os << "#include \"utils.h\"" << endl;
  os << "#include \"host.hpp\"" << endl;
  os << "#include \"neuron.cpp\"" << endl;
  if (model->synapseGrpN>0) os << "#include \"synapse.cpp\"" << endl;
  os << "#ifndef RAND" << endl;
