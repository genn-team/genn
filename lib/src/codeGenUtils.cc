    ...
    )
{
    ...
    substitute(wCode, "$(sT_post)", devPrefix + "sT" + sg->getTrgNeuronGroup()->getName() + "[" + trgNeuronModel->getQueueOffset(devPrefix) + postIdx + "]");
    for(const auto &v : trgNeuronModel->getVars()) {
        if (sg->getTrgNeuronGroup()->isVarQueueRequired(v.first)) {
            substitute(wCode, "$(" + v.first + "_post)",
                       devPrefix + v.first + sg->getTrgNeuronGroup()->getName() + "[" + trgNeuronModel->getQueueOffset(devPrefix) + postIdx + "]");
        }
        else {
            substitute(wCode, "$(" + v.first + "_post)",
