...
namespace GENN_PREFERENCES {    
    ...
    extern int optimizeCode; //!< Request speed-optimized code, at the expense of floating-point accuracy
    extern int debugCode; //!< Request debug data to be embedded in the generated code
    extern int showPtxInfo; //!< Request that PTX assembler information be displayed for each CUDA kernel during compilation
    extern int smVersionFile; //!< Request a Makefile include file (sm_version.mk), containing architecture flags (-arch=sm_**), for use with the NVCC compiler
