...
def generateBackend(swigPath, folder, namespace):
    ...
    with SwigModuleGenerator(namespace + "Backend", os.path.join(swigPath, namespace + "Backend.i")) as mg:
        ...
        with SwigInlineScope(mg):
            mg.write('CodeGenerator::' + namespace + '::Backend create_backend(const ModelSpecInternal &model, const std::string &outputPath, plog::Severity backendLevel, int localHostID, const CodeGenerator::' + namespace + '::Preferences &preferences)\n'
                     '{\n'
                     '  auto *consoleAppender = new plog::ConsoleAppender<plog::TxtFormatter>;\n'
                     '  return Optimiser::createBackend(model, filesystem::path(outputPath), backendLevel, consoleAppender, localHostID, preferences);\n'
                     '}\n\n'
