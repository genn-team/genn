...
public:
    ...
    VarLocation getVarLocation(size_t index) const{ return m_NG.getVarLocation(index); }
    
    Models::Base::VarVec getVars() const{ return m_NG.getNeuronModel()->getVars(); }

    const std::vector<Models::VarInit> &getVarInitialisers() const{ return m_NG.getVarInitialisers(); }
...
public:
    ...
    VarLocation getEGPLocation(size_t index) const{ return m_NG.getExtraGlobalParamLocation(index); }
