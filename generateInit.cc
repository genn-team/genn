...
void genInitNeuronVarCode(CodeStream &os, const CodeGenerator::Backends::Base &backend, const Substitutions &kernelSubs, const NewModels::Base::StringPairVec &vars,
                          ...
                          I getVarInitialiser, M getVarMode, Q isVarQueueRequired)
{
    for (size_t k = 0; k < vars.size(); k++) {
        ...
        if(!varInit.getSnippet()->getCode().empty()) {
            ...
            backend.genVariableInit(os, varMode, count, kernelSubs,
...
{
    genInitNeuronVarCode(os, backend, kernelSubs, vars, count, 0, popName, ftype, getVarInitialiser, getVarMode,
                         [](size_t){ return false; });
}
}   // Anonymous namespace
...
void CodeGenerator::generateInit(CodeStream &os, const NNmodel &model, const Backends::Base &backend)
{
    ...
    backend.genInitKernel(os, model,
        [&backend, &model](CodeStream &os, const NeuronGroup &ng, const Substitutions &kernelSubs)
        {
            // Initialise neuron variables
            genInitNeuronVarCode(os, backend, kernelSubs, ng.getNeuronModel()->getVars(), ng.getNumNeurons(), ng.getNumDelaySlots(),
            ...
            for(const auto &m : ng.getMergedInSyn()) {
                ...
                // Generate target-specific code to initialise variable
                backend.genVariableInit(os, sg->getInSynVarMode(), ng.getNumNeurons(), kernelSubs,
                ...
                if(sg->isDendriticDelayRequired()) {
                    backend.genVariableInit(os, sg->getDendriticDelayVarMode(), ng.getNumNeurons(), kernelSubs,
                        [&backend, &model, sg](CodeStream &os, Substitutions &varSubs)
                        {
                ...
                if(sg->getMatrixType() & SynapseMatrixWeight::INDIVIDUAL_PSM) {
                    genInitNeuronVarCode(os, backend, kernelSubs, sg->getPSModel()->getVars(), ng.getNumNeurons(), sg->getName(), model.getPrecision(),
            ...
            for(const auto *s : ng.getInSyn()) {
                genInitNeuronVarCode(os, backend, kernelSubs, s->getWUModel()->getPostVars(), ng.getNumNeurons(), s->getTrgNeuronGroup()->getNumDelaySlots(), s->getName(), model.getPrecision(),
            ...
            for(const auto *s : ng.getOutSyn()) {
                ...
                genInitNeuronVarCode(os, backend, kernelSubs, s->getWUModel()->getPreVars(), ng.getNumNeurons(), s->getSrcNeuronGroup()->getNumDelaySlots(), s->getName(), model.getPrecision(),
            ...
            for (auto const *cs : ng.getCurrentSources()) {
                genInitNeuronVarCode(os, backend, kernelSubs, cs->getCurrentSourceModel()->getVars(), ng.getNumNeurons(), cs->getName(), model.getPrecision(),
