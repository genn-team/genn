...
postVarsInSimCode::postVarsInSimCode()
{
  allocateMem();
  initialize();
  init_synapses();
...
void postVarsInSimCode::init_synapses() {
    theW= new float*[10];
    theW[0]= wsyn0;
    theW[1]= wsyn1;
    theW[2]= wsyn2;
...
void postVarsInSimCode::init_neurons() {
    for (int i= 0; i < 10; i++) {
	shiftpre[i]= i*10.0f;
	shiftpost[i]= i*10.0f;
    }
...
int main(int argc, char *argv[])
{
  if (argc != 4)
  {
    cerr << "usage: postVarsInSimCodeSim <GPU = 1, CPU = 0> <output label> <write output files? 0/1>" << endl;
    return EXIT_FAILURE;
  }
  ...
  cout << "# test postVarsInSimCode: Result " << result << endl;
