...
#include "testPostVarsInPostLearn.h"
#include "postVarsInPostLearn_CODE/definitions.h"
#include "postVarsInPostLearn_CODE/runner.cc"



postVarsInPostLearn::postVarsInPostLearn()
{
  allocateMem();
  initialize();
  init_synapses();
...
void postVarsInPostLearn::init_synapses() {
    theW= new float*[10];
    theW[0]= wsyn0;
    theW[1]= wsyn1;
    theW[2]= wsyn2;
...
void postVarsInPostLearn::init_neurons() {
    for (int i= 0; i < 10; i++) {
	shiftpre[i]= i*10.0f;
 	shiftpost[i]= i*10.0f;
   }
...
void postVarsInPostLearn::run(int which)
{
  if (which == GPU)
  {
    stepTimeGPU();
...
int main(int argc, char *argv[])
{
  if (argc != 4)
  {
    cerr << "usage: postVarsInPostLearnSim <GPU = 1, CPU = 0> <output label> <write output files? 0/1>" << endl;
    return EXIT_FAILURE;
  }

  postVarsInPostLearn *sim = new postVarsInPostLearn();
  ...
  {      
	  ...
	  for (int j= 0; j < 10; j++) { // for all pre-synaptic neurons 
		  ...
		  if ((t > 2.0001) && (fmod(t-DT+5e-5,2.0f) < 1e-4)) {
  ...
  cout << "# test postVarsInPostLearn: Result " << result << endl;
