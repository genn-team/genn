...
#include "testPostVarsInSynapseDynamics_sparse.h"
#include "postVarsInSynapseDynamics_sparse_CODE/definitions.h"
#include "postVarsInSynapseDynamics_sparse_CODE/runner.cc"



postVarsInSynapseDynamics_sparse::postVarsInSynapseDynamics_sparse()
{
  allocateMem();
  initialize();
  init_synapses();
...
void postVarsInSynapseDynamics_sparse::init_synapses() {
...
void postVarsInSynapseDynamics_sparse::init_neurons() {
    for (int i= 0; i < 10; i++) {
	shiftpre[i]= i*10.0f;
	shiftpost[i]= i*10.0f;
    }
...
int main(int argc, char *argv[])
{
  if (argc != 4)
  {
    cerr << "usage: postVarsInSynapseDynamics_sparseSim <GPU = 1, CPU = 0> <output label> <write output files? 0/1>" << endl;
    return EXIT_FAILURE;
  }
  ...
  cout << "# test postVarsInSynapseDynamics_sparse: Result " << result << endl;
