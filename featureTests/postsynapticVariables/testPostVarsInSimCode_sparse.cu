...
#include "testPostVarsInSimCode_sparse.h"
#include "postVarsInSimCode_sparse_CODE/definitions.h"
#include "postVarsInSimCode_sparse_CODE/runner.cc"



postVarsInSimCode_sparse::postVarsInSimCode_sparse()
{
  allocateMem();
  initialize();
  init_synapses();
...
void postVarsInSimCode_sparse::init_synapses() {
...
void postVarsInSimCode_sparse::init_neurons() {
    for (int i= 0; i < 10; i++) {
	shiftpre[i]= i*10.0f;
	shiftpost[i]= i*10.0f;
    }
...
void postVarsInSimCode_sparse::run(float t, int which)
{
  if (which == GPU)
  {
    stepTimeGPU(t);
...
int main(int argc, char *argv[])
{
  if (argc != 4)
  {
    cerr << "usage: postVarsInSimCode_sparseSim <GPU = 1, CPU = 0> <output label> <write output files? 0/1>" << endl;
    return EXIT_FAILURE;
  }

  float t = 0.0f;
  postVarsInSimCode_sparse *sim = new postVarsInSimCode_sparse();
  ...
  cout << "# test postVarsInSimCode_sparse: Result " << result << endl;
