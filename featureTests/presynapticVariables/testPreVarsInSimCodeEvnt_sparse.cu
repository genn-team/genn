...
#include "testPreVarsInSimCodeEvnt_sparse.h"
#include "preVarsInSimCodeEvnt_sparse_CODE/definitions.h"
#include "preVarsInSimCodeEvnt_sparse_CODE/runner.cc"



preVarsInSimCodeEvnt_sparse::preVarsInSimCodeEvnt_sparse()
{
  allocateMem();
  initialize();
  init_synapses();
...
void preVarsInSimCodeEvnt_sparse::init_synapses() {
...
void preVarsInSimCodeEvnt_sparse::init_neurons() {
    for (int i= 0; i < 10; i++) {
	shiftpre[i]= i*10.0f;
    }
    copyStateToDevice();
...
void preVarsInSimCodeEvnt_sparse::run(int which)
{
  if (which == GPU)
  {
    stepTimeGPU();
...
int main(int argc, char *argv[])
{
  if (argc != 4)
  {
    cerr << "usage: testPreVarsInSimCodeEvnt_sparse <GPU = 1, CPU = 0> <output label> <write output files? 0/1>" << endl;
    return EXIT_FAILURE;
  }

  preVarsInSimCodeEvnt_sparse *sim = new preVarsInSimCodeEvnt_sparse();
  ...
  cout << "# test preVarsInSimCodeEvnt_sparse: Result " << result << endl;
