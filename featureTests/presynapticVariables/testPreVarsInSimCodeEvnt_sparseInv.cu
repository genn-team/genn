...
#ifndef TESTPREVARSINSIMCODEEVNT_CU
#define TESTPREVARSINSIMCODEEVNT_CU
...
#include "testPreVarsInSimCodeEvnt_sparseInv.h"
#include "preVarsInSimCodeEvnt_sparseInv_CODE/definitions.h"
#include "preVarsInSimCodeEvnt_sparseInv_CODE/runner.cc"



preVarsInSimCodeEvnt_sparseInv::preVarsInSimCodeEvnt_sparseInv()
{
  allocateMem();
  initialize();
  init_synapses();
...
void preVarsInSimCodeEvnt_sparseInv::init_synapses() {
...
void preVarsInSimCodeEvnt_sparseInv::init_neurons() {
    for (int i= 0; i < 10; i++) {
	shiftpre[i]= i*10.0f;
    }
    copyStateToDevice();
...
void preVarsInSimCodeEvnt_sparseInv::run(int which)
{
  if (which == GPU)
  {
    stepTimeGPU();
...
int main(int argc, char *argv[])
{
  if (argc != 4)
  {
    cerr << "usage: testPreVarsInSimCodeEvnt_sparseInv <GPU = 1, CPU = 0> <output label> <write output files? 0/1>" << endl;
    return EXIT_FAILURE;
  }

  preVarsInSimCodeEvnt_sparseInv *sim = new preVarsInSimCodeEvnt_sparseInv();
  ...
  cout << "# test preVarsInSimCodeEvnt_sparseInv: Result " << result << endl;
...
#endif // TESTPREVARSINSIMCODEEVNT_CU
