...
#include "testPreVarsInSimCodeEvnt.h"
#include "preVarsInSimCodeEvnt_CODE/definitions.h"
#include "preVarsInSimCodeEvnt_CODE/runner.cc"



preVarsInSimCodeEvnt::preVarsInSimCodeEvnt()
{
  allocateMem();
  initialize();
  init_synapses();
...
void preVarsInSimCodeEvnt::init_synapses() {
    theW= new float*[10];
    theW[0]= wsyn0;
    theW[1]= wsyn1;
    theW[2]= wsyn2;
...
void preVarsInSimCodeEvnt::init_neurons() {
    for (int i= 0; i < 10; i++) {
	shiftpre[i]= i*10.0f;
    }
    copyStateToDevice();
...
void preVarsInSimCodeEvnt::run(int which)
{
  if (which == GPU)
  {
    stepTimeGPU();
...
int main(int argc, char *argv[])
{
  if (argc != 4)
  {
    cerr << "usage: testPreVarsInSimCodeEvnt <GPU = 1, CPU = 0> <output label> <write output files? 0/1>" << endl;
    return EXIT_FAILURE;
  }

  preVarsInSimCodeEvnt *sim = new preVarsInSimCodeEvnt();
  ...
  cout << "# test preVarsInSimCodeEvnt: Result " << result << endl;
