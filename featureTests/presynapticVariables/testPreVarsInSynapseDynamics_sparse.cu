...
#ifndef TESTPREVARSINSYNAPSEDYNAMICS_SPARSE_CU
#define TESTPREVARSINSYNAPSEDYNAMICS_SPARSE_CU
...
#include "testPreVarsInSynapseDynamics_sparse.h"
#include "preVarsInSynapseDynamics_sparse_CODE/definitions.h"
#include "preVarsInSynapseDynamics_sparse_CODE/runner.cc"



preVarsInSynapseDynamics_sparse::preVarsInSynapseDynamics_sparse()
{
  allocateMem();
  initialize();
  init_synapses();
...
void preVarsInSynapseDynamics_sparse::init_synapses() {
    ...
    initpreVarsInSynapseDynamics_sparse();
}


void preVarsInSynapseDynamics_sparse::init_neurons() {
    for (int i= 0; i < 10; i++) {
	shiftpre[i]= i*10.0f;
    }
    copyStateToDevice();
...
void preVarsInSynapseDynamics_sparse::run(int which)
{
  if (which == GPU)
  {
    stepTimeGPU();
...
int main(int argc, char *argv[])
{
  if (argc != 4)
  {
    cerr << "usage: preVarsInSynapseDynamics_sparseSim <GPU = 1, CPU = 0> <output label> <write output files? 0/1>" << endl;
    return EXIT_FAILURE;
  }

  preVarsInSynapseDynamics_sparse *sim = new preVarsInSynapseDynamics_sparse();
  ...
  cout << "# test preVarsInSynapseDynamics_sparse: Result " << result << endl;
...
#endif // TESTPREVARSINSYNAPSEDYNAMICS_SPARSE_CU
