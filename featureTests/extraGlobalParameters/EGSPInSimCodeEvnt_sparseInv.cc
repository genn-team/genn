...
{
  initGeNN();
  model.setName("EGSPInSimCodeEvnt_sparseInv");
  ...
  s.extraGlobalSynapseKernelParameters.push_back(tS("thresh"));
  s.extraGlobalSynapseKernelParameterTypes.push_back(tS("scalar"));  
  s.evntThreshold= tS("(fmod($(x_pre),$(thresh)) < 1e-4)");
