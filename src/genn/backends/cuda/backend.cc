                              ...
                              HostHandler pushEGPHandler) const
{
    ...
    os << ")" << std::endl;
    {
        ...
        genParallelGroup<NeuronUpdateGroupMerged>(os, kernelSubs, modelMerged.getMergedNeuronUpdateGroups(), idStart,
                               ...
                               HostHandler pushEGPHandler) const
{
    ...
    genMergedKernelDataStructures(os, m_KernelBlockSizes[KernelPresynapticUpdate], totalConstMem, modelMerged.getMergedPresynapticUpdateGroups(), 
    ...
    if(!modelMerged.getMergedPresynapticUpdateGroups().empty()) {
        os << "extern \"C\" __global__ void " << KernelNames[KernelPresynapticUpdate] << "(" << model.getTimePrecision() << " t)" << std::endl; // end of synapse kernel header
        {
            ...
            genParallelGroup<PresynapticUpdateGroupMerged>(os, kernelSubs, modelMerged.getMergedPresynapticUpdateGroups(), idPresynapticStart,
    ...
    if(!modelMerged.getMergedPostsynapticUpdateGroups().empty()) {
        os << "extern \"C\" __global__ void " << KernelNames[KernelPostsynapticUpdate] << "(" << model.getTimePrecision() << " t)" << std::endl;
        {
            ...
            genParallelGroup<PostsynapticUpdateGroupMerged>(os, kernelSubs, modelMerged.getMergedPostsynapticUpdateGroups(), idPostsynapticStart,
    ...
    if(!modelMerged.getMergedSynapseDynamicsGroups().empty()) {
        os << "extern \"C\" __global__ void " << KernelNames[KernelSynapseDynamicsUpdate] << "(" << model.getTimePrecision() << " t)" << std::endl; // end of synapse kernel header
        {
            ...
            genParallelGroup<SynapseDynamicsGroupMerged>(os, kernelSubs, modelMerged.getMergedSynapseDynamicsGroups(), idSynapseDynamicsStart,
                      ...
                      SynapseSparseInitGroupMergedHandler sgSparseInitHandler, HostHandler initPushEGPHandler, HostHandler initSparsePushEGPHandler) const
{
    ...
    size_t idInitStart = 0;
    {
        ...
        genParallelGroup<NeuronInitGroupMerged>(os, kernelSubs, modelMerged.getMergedNeuronInitGroups(), idInitStart,
        ...
        genParallelGroup<SynapseDenseInitGroupMerged>(os, kernelSubs, modelMerged.getMergedSynapseDenseInitGroups(), idInitStart,
        ...
        genParallelGroup<SynapseConnectivityInitGroupMerged>(os, kernelSubs, modelMerged.getMergedSynapseConnectivityInitGroups(), idInitStart,
            [this](const SynapseGroupInternal &sg){ return padSize(getNumConnectivityInitThreads(sg), m_KernelBlockSizes[KernelInitialize]); },
            [this, sgSparseRowConnectHandler, sgSparseColConnectHandler](CodeStream &os, const SynapseConnectivityInitGroupMerged &sg, Substitutions &popSubs)
            {
                ...
                // Otherwise, if there is column building code
                    ...
                    os << "if(" << popSubs["id"] << " < group->numTrgNeurons)";
                    {
                        ...
                        if(sg.getArchetype().getMatrixType() & SynapseMatrixConnectivity::BITMASK) {
                            ...
                            // Build function template to set correct bit in bitmask
                            popSubs.addFuncSubstitution("addSynapse", 1,
                                                        "atomicOr(&group->gp[(colStartGID + ($(0) * group->rowStride)) / 32], 0x80000000 >> ((colStartGID + ($(0) * group->rowStride)) & 31))");
                        }
                        // Otherwise, if synapse group has ragged connectivity
                        else if(sg.getArchetype().getMatrixType() & SynapseMatrixConnectivity::SPARSE) {
    ...
    if(!modelMerged.getMergedSynapseSparseInitGroups().empty()) {
        os << "extern \"C\" __global__ void " << KernelNames[KernelInitializeSparse] << "()";
        {
            ...
            genParallelGroup<SynapseSparseInitGroupMerged>(os, kernelSubs, modelMerged.getMergedSynapseSparseInitGroups(), idSparseInitStart,
