                        ...
                        unsigned int batchSize, double dt, bool trueSpike) const
{
    ...
    env.print("if (spike < $(_src_spk_cnt" + eventSuffix + ")[" + sg.getPreSlot(delay, batchSize) + "])");
    {
        ...
        }
        {
            ...
            if(trueSpike) {
                sg.generateSpikeUpdate(backend, synEnv, batchSize, dt, false);
            }
            else {
                sg.generateSpikeEventUpdate(backend, synEnv, batchSize, dt, false);
            }
                                         ...
                                         const Type::TypeContext &typeContext) const
{
    ...
    // Divide num neurons by vector width
    return ceilDivide(sg.getTrgNeuronGroup()->getNumNeurons(), vectorWidth);
}
//----------------------------------------------------------------------------
                                                      ...
                                                      const Type::TypeContext &typeContext) const
{
    ...
    // Pad num neurons to vector width
    return padSize(sg.getTrgNeuronGroup()->getNumNeurons(), vectorWidth);
}
//----------------------------------------------------------------------------
                                      ...
                                      const Type::TypeContext &typeContext) const
{
    ...
    return ((backend.getPresynapticUpdateVectorWidth(sg, typeContext) > 1)
            ...
            && (!sg.isDendriticOutputDelayRequired() && (sg.getMatrixType() & SynapseMatrixConnectivity::DENSE)));
}
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
size_t PostSpanVectorised::getSharedMemoryPerThread(const PresynapticUpdateGroupMerged&, const BackendSIMT&) const
{
    return 0;
}
//----------------------------------------------------------------------------
                                   ...
                                   unsigned int batchSize, double dt, bool trueSpike) const
{
    ...
    env.getStream() << "for (unsigned int r = 0; r < numSpikeBlocks; r++)";
    {
        ...
        env.getStream() << "for (unsigned int j = 0; j < numSpikesInBlock; j++)";
        {
            ...
            spikeEnv.printLine("// Vectorised");
            spikeEnv.printLine("const uint32_t numVectors = ($(_row_stride) + " + vectorWidthMinus1Str + ") / " + vectorWidthStr + ";");

            spikeEnv.printLine("// only work on existing neurons");
            spikeEnv.print("if ($(id) < numVectors)");
            {
                ...
                EnvironmentLocalVectorVarCache<SynapseWUVarAdapter, PresynapticUpdateGroupMerged> wuVarEnv(
                    sg, sg, backend, sg.getTypeContext(), idEnv, "", "l",
                    [batchSize, &sg](const std::string&, VarAccess a, bool)
                    {
                ...
                for(size_t i = 0; i < vectorWidth; i++) {
                    ...
                    if(trueSpike) {
                        sg.generateSpikeUpdate(backend, synEnv, batchSize, dt, true);
                    }
                    else {
                                  ...
                                  unsigned int batchSize, double dt, bool trueSpike) const
{
    ...
    groupEnv.print("if ($(_spike) < $(_src_spk_cnt" + eventSuffix + ")[" + sg.getPreSlot(delay, batchSize) + "])");
    {
        ...
        CodeStream preUpdate(preUpdateStream);
        {
            ...
            if(trueSpike) {
                sg.generateSpikeUpdate(backend, preUpdateEnv, batchSize, dt, false);
            }
            else {
                sg.generateSpikeEventUpdate(backend, preUpdateEnv, batchSize, dt, false);
            }
        }
        ...
        env.getStream() << "for (unsigned int j = 0; j < numSpikesInBlock; j++)";
        {
            ...
            spikeEnv.print("if ($(id) < rowWords)");
            {
                ...
                spikeEnv.getStream() << "while(connectivityWord != 0)";
                {
                    ...
                    if(trueSpike) {
                        sg.generateSpikeUpdate(backend, postEnv, batchSize, dt, false);
                    }
                    else {
                        sg.generateSpikeEventUpdate(backend, postEnv, batchSize, dt, false);
                    }
                                 ...
                                 unsigned int batchSize, double dt, bool trueSpike) const
{
    ...
    CodeStream preUpdate(preUpdateStream);
    {
        ...
        // Generate spike update
        if(trueSpike) {
            sg.generateSpikeUpdate(backend, preUpdateEnv, 1, dt, false);
        }
        else {
            sg.generateSpikeEventUpdate(backend, preUpdateEnv, 1, dt, false);
        }
    }
