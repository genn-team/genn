...
void BackendSIMT::genNeuronSpikeQueueUpdateKernel(EnvironmentExternalBase &env, ModelSpecMerged &modelMerged, size_t &idStart) const
{
    ...
    modelMerged.genMergedNeuronSpikeQueueUpdateGroups(
        *this, 
        [&env, &idStart, batchSize, this](const auto &n)
        {
            if(idStart == 0) {
            ...
            }
            {
                ...
                // Create matching environment
                EnvironmentGroupMergedField<NeuronSpikeQueueUpdateGroupMerged> neuronEnv(env, batchSize);
                genNeuronIndexCalculation(neuronEnv, batchSize);
...
void BackendSIMT::genNeuronUpdateKernel(EnvironmentExternalBase &env, ModelSpecMerged &modelMerged, size_t &idStart) const
{
    ...
    genParallelGroup<NeuronUpdateGroupMerged>(
        neuronEnv, modelMerged, idStart, &ModelSpecMerged::genMergedNeuronUpdateGroups,
        [this](const NeuronGroupInternal &ng) { return padKernelSize(ng.getNumNeurons(), KernelNeuronUpdate); },
        [batchSize, &modelMerged, this](EnvironmentExternalBase &popEnv, NeuronUpdateGroupMerged &ng)
        {
            ...
            if(!ng.getArchetype().getNeuronModel()->getThresholdConditionCode().empty()) {
                groupEnv.getStream() << "if(" << getThreadID() << " == 0)";
                {
                    ...
                    {
                        CodeStream::Scope b(groupEnv.getStream());
                        groupEnv.getStream() << "shPosSpk = " << getAtomic(Type::Uint32) << "(&group->spkCnt";
                        if(ng.getArchetype().isDelayRequired() && ng.getArchetype().isTrueSpikeRequired()) {
                            groupEnv.getStream() << "[*" << groupEnv["_spk_que_ptr"];
                            if(batchSize > 1) {
                                groupEnv.getStream() << " + (batch * " << ng.getArchetype().getNumDelaySlots() << ")";
                            }
                            groupEnv.getStream() << "], shSpkCount);" << std::endl;
                        }
                        else {
                            groupEnv.getStream() << "[" << ((batchSize > 1) ? "batch" : "0") << "], shSpkCount);" << std::endl;
                        }
                    }
                } 
                genSharedMemBarrier(groupEnv.getStream());
            ...
            env.print("if($(id) < $(num_neurons))");
            {
                ...
                if(isPopulationRNGInitialisedOnDevice() && ng.getArchetype().isSimRNGRequired()) {
                    ...
                    // Otherwise, loop through batches and initialise independent RNGs using GLOBAL thread id as basis of sequence
                        ...
                        {
                            CodeStream::Scope b(groupEnv.getStream());
                            genPopulationRNGInit(groupEnv.getStream(), printSubs("$(_rng)[(b * $(num_neurons)) + $(id)]", groupEnv), 
