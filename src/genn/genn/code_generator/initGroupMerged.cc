...
namespace
{
void genVariableFill(const BackendBase &backend, EnvironmentExternalBase &env, const std::string &target, const std::string &value, const std::string &idx, const std::string &stride,
                     VarAccessDim varDims, const Type::ResolvedType &type, const Type::ResolvedType &storageType,
                     unsigned int batchSize, std::optional<unsigned int> numDelaySlots = std::nullopt)
{
    ...
    if(numValues == 1) {
        env.print("$(" + target + ")[$(" + idx + ")] = ");
        if(type == storageType) {
            env.printLine(value + ";");
        }
        else {
            env.printLine(backend.getTypeToStorageConversion(type, storageType, value) + ";");
        }
    }
    // Otherwise
    else {
...
//--------------------------------------------------------------------------
void genScalarFill(const BackendBase &backend, EnvironmentExternalBase &env, const std::string &target, const std::string &value,
                   VarAccessDim varDims, const Type::ResolvedType &type, const Type::ResolvedType &storageType,
                   unsigned int batchSize, std::optional<unsigned int> numDelaySlots = std::nullopt)
{
    // Determine number of values to fill in each thread
                   ...
                   NeuronInitGroupMerged &ng, const std::string &fieldSuffix, bool trueSpike, bool delayRequired, unsigned int batchSize)
{
    ...
    backend.genVariableInit(env, "num_neurons", "id",
        [batchSize, numDelaySlots, &backend, &ng] (EnvironmentExternalBase &varEnv)
        {
            genVariableFill(backend, varEnv, "_event", "0", "id", "$(_stride)", 
                            VarAccessDim::BATCH | VarAccessDim::ELEMENT, Type::Uint32, Type::Uint32,
    ...
    backend.genPopVariableInit(env,
        [batchSize, numDelaySlots, &backend, &ng] (EnvironmentExternalBase &spikeCountEnv)
        {
            genScalarFill(backend, spikeCountEnv, "_event_cnt", "0", 
                          VarAccessDim::BATCH | VarAccessDim::ELEMENT, Type::Uint32, Type::Uint32,
                          batchSize, numDelaySlots);
        });
}
...
{
    EnvironmentGroupMergedField<G, NeuronInitGroupMerged> timeEnv(env, group, fieldGroup, backend);
    ...
    backend.genVariableInit(timeEnv, "num_neurons", "id",
        [batchSize, numDelaySlots, &backend, &group] (EnvironmentExternalBase &varEnv)
        {
            genVariableFill(backend, varEnv, "_time", "-TIME_MAX", "id", "$(_stride)", 
                            VarAccessDim::BATCH | VarAccessDim::ELEMENT, group.getTimeType(), group.getTimeType(),
                            batchSize, numDelaySlots);
        });
}
//------------------------------------------------------------------------
                      ...
                      const std::string &varName, bool delayRequired, unsigned int batchSize)
{
    ...
    backend.genVariableInit(env, "num_neurons", "id",
        [batchSize, numDelaySlots, varName, &backend, &group] (EnvironmentExternalBase &varEnv)
        {
            genVariableFill(backend, varEnv, varName, "-TIME_MAX", "id", "$(_stride)", 
                            VarAccessDim::BATCH | VarAccessDim::ELEMENT, group.getTimeType(), group.getTimeType(),
                            batchSize, numDelaySlots);
        });
}
...
void genInitNeuronVarCode(const BackendBase &backend, EnvironmentExternalBase &env,
                          ...
                          const std::string &count, const std::string &stride, unsigned int batchSize)
{
    ...
    for (const auto &var : adaptor.getDefs()) {
        ...
        const auto resolvedStorageType = var.storageType.resolve(group.getTypeContext());
        ...
        if (!Utils::areTokensEmpty(varInit.getCodeTokens())) {
            ...
            EnvironmentGroupMergedField<G, F> varEnv(env, group, fieldGroup, backend);
            ...
            varEnv.addField(resolvedStorageType.createPointer(), "_value", var.name + fieldSuffix,
            ...
            if (varDims & VarAccessDim::ELEMENT) {
                backend.genVariableInit(
                    varEnv, count, "id",
                    [&adaptor, &backend, &count, &fieldGroup, &group, &resolvedType, 
                     &resolvedStorageType, &stride, &var, &varInit, batchSize, varDims]
                    (EnvironmentExternalBase &env)
                    {
                        // Generate initial value into temporary variable
                        EnvironmentGroupMergedField<G, F> varInitEnv(env, group, fieldGroup, backend);
                        ...
                        genVariableFill(backend, varInitEnv, "_value", "$(value)", "id", "$(" + stride + ")",
                                        varDims, resolvedType, resolvedStorageType,
            ...
            // Otherwise
                ...
                backend.genPopVariableInit(
                    varEnv,
                    [&adaptor, &backend, &fieldGroup, &group, &resolvedType,
                     &resolvedStorageType, &var, &varInit, batchSize, varDims]
                    (EnvironmentExternalBase &env)
                    {
                        // Generate initial value into temporary variable
                        EnvironmentGroupMergedField<G, F> varInitEnv(env, group, fieldGroup, backend);
                        ...
                        // Fill value across all delay slots and batches
                        genScalarFill(backend, varInitEnv, "_value", "$(value)", 
                                      varDims, resolvedType, resolvedStorageType,
                                      batchSize, adaptor.getNumVarDelaySlots(var.name));
                    });
            }
...
void genInitNeuronVarCode(const BackendBase &backend, EnvironmentExternalBase &env,
                          G &group, const std::string &fieldSuffix, const std::string &count,
                          const std::string &stride, unsigned int batchSize)
{
    genInitNeuronVarCode<A, G, G>(backend, env, group, group, fieldSuffix, 
                                  count, stride, batchSize);
}
//------------------------------------------------------------------------
                      ...
                      V genSynapseVariableRowInitFn)
{
    ...
    for (const auto &var : adaptor.getDefs()) {
        ...
        if(!Utils::areTokensEmpty(varInit.getCodeTokens()) && (varInit.isKernelRequired() == kernel)) {
            ...
            // Substitute in parameters and derived parameters for initialising variables
            EnvironmentGroupMergedField<G> varEnv(env, group, backend);
            ...
            genSynapseVariableRowInitFn(varEnv,
                [&adaptor, &backend, &group, &resolvedType, &resolvedStorageType,
                 &stride, &var, &varInit, batchSize]
                (EnvironmentExternalBase &env)
                {
                    // Generate initial value into temporary variable
                    EnvironmentGroupMergedField<G> varInitEnv(env, group, backend);
                    varInitEnv.getStream() << resolvedType.getName() << " initVal;" << std::endl;
                    varInitEnv.add(resolvedType, "value", "initVal");
                    ...
                    genVariableFill(backend, varInitEnv, "_value", "$(value)", "id_syn", stride,
                                    adaptor.getVarDims(var), resolvedType, resolvedStorageType, 
                                    batchSize);
                });
        }
    }
                                               ...
                                               NeuronInitGroupMerged &ng, unsigned int batchSize)
{
    ...
    EnvironmentGroupMergedField<SynSpike, NeuronInitGroupMerged> groupEnv(env, *this, ng, backend);
                                                    ...
                                                    NeuronInitGroupMerged &ng, unsigned int batchSize)
{
    ...
    // Create environment for group
    EnvironmentGroupMergedField<SynSpikeEvent, NeuronInitGroupMerged> groupEnv(env, *this, ng, backend);
                                               ...
                                               NeuronInitGroupMerged &ng, unsigned int batchSize)
{
    ...
    // Create environment for group
    EnvironmentGroupMergedField<InSynPSM, NeuronInitGroupMerged> groupEnv(env, *this, ng, backend);
    ...
    backend.genVariableInit(groupEnv, "num_neurons", "id",
        [batchSize, &backend, this] (EnvironmentExternalBase &varEnv)
        {
            genVariableFill(backend, varEnv, "_out_post", Type::writeNumeric(0.0, getScalarType()), 
                            "id", "$(num_neurons)", VarAccessDim::BATCH | VarAccessDim::ELEMENT,
                            getScalarType(), getScalarType(), batchSize);
    ...
    if(getArchetype().isDendriticOutputDelayRequired()) {
        ...
        backend.genVariableInit(groupEnv, "num_neurons", "id",
            [batchSize, &backend, this](EnvironmentExternalBase &varEnv)
            {
                genVariableFill(backend, varEnv, "_den_delay", Type::writeNumeric(0.0, getScalarType()),
                                "id", "$(num_neurons)", VarAccessDim::BATCH | VarAccessDim::ELEMENT, 
                                getScalarType(), getScalarType(), batchSize, getArchetype().getMaxDendriticDelayTimesteps());
            });
    ...
    genInitNeuronVarCode<SynapsePSMVarAdapter, NeuronInitGroupMerged>(
        backend, groupEnv, *this, ng, fieldSuffix, "num_neurons", "_stride", batchSize);
}
...
{
    // Create environment for group
    EnvironmentGroupMergedField<OutSynPreOutput, NeuronInitGroupMerged> groupEnv(env, *this, ng, backend);
    ...
    backend.genVariableInit(groupEnv, "num_neurons", "id",
                            [batchSize, &backend, this] (EnvironmentExternalBase &varEnv)
                            {
                                genVariableFill(backend, varEnv, "_out_pre", Type::writeNumeric(0.0, getScalarType()),
                                                "id", "$(num_neurons)", VarAccessDim::BATCH | VarAccessDim::ELEMENT,
                                                getScalarType(), getScalarType(), batchSize);
                            });
}
...
{
    // Create environment for group
    EnvironmentGroupMergedField<NeuronInitGroupMerged> groupEnv(env, *this, backend);
...
{
    // Create environment for group
    EnvironmentGroupMergedField<SynapseInitGroupMerged> groupEnv(env, *this, backend);
...
{
    // Create environment for group
    EnvironmentGroupMergedField<SynapseConnectivityInitGroupMerged> groupEnv(env, *this, backend);
...
void SynapseConnectivityHostInitGroupMerged::generateInit(const BackendBase &backend, EnvironmentExternalBase &env)
{
    ...
    envAssert.getStream() << "for(unsigned int g = 0; g < " << getGroups().size() << "; g++)";
    {
        ...
        // Create environment for group
        EnvironmentGroupMergedField<SynapseConnectivityHostInitGroupMerged> groupEnv(envAssert, *this, backend);
        const auto &connectInit = getArchetype().getSparseConnectivityInitialiser();
...
{
    // Create environment for group
    EnvironmentGroupMergedField<CustomUpdateInitGroupMerged> groupEnv(env, *this, backend);
    ...
    genInitNeuronVarCode<CustomUpdateVarAdapter>(backend, groupEnv, *this, "", 
                                                 "num_neurons", "_stride", updateBatchSize);
}
...
{
    EnvironmentGroupMergedField<CustomWUUpdateInitGroupMerged> groupEnv(env, *this, backend);
    ...
    genInitWUVarCode<CustomUpdateVarAdapter>(
        backend, groupEnv, *this, stride, updateBatchSize, false,
        [&backend, kernel, this](EnvironmentExternalBase &varInitEnv, BackendBase::HandlerEnv handler)
        {
            if (kernel) {
...
{
    EnvironmentGroupMergedField<CustomWUUpdateSparseInitGroupMerged> groupEnv(env, *this, backend);
    ...
    genInitWUVarCode<CustomUpdateVarAdapter>(
        backend, groupEnv, *this, "$(num_pre) * $(_row_stride)", updateBatchSize, false,
        [&backend](EnvironmentExternalBase &varInitEnv, BackendBase::HandlerEnv handler)
        {
