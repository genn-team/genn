        ...
        virtual void visit(const Expression::Identifier &variable) final
        {
            ...
            if (type.isFunction()) {
                ...
                if (type.getFunction().hasFlag(Type::FunctionFlags::VARIADIC)) {
                    ...
                    size_t varPlaceholderPos = name.find(variadicPlaceholder);
                    if (varPlaceholderPos != std::string::npos) {
                        ...
                        if (!variadicArguments.empty()) {
                            variadicArguments = variadicArguments.substr(0, variadicArguments.length() - 2);
                        }
                        // Extend replacement to include any trailing comma and whitespace
                        size_t endReplacePos = varPlaceholderPos + variadicPlaceholder.length();
                        while(endReplacePos < name.size() && (isspace(name[endReplacePos]) || name[endReplacePos] == ','))
                            endReplacePos++;
                        name.replace(varPlaceholderPos, endReplacePos - varPlaceholderPos, variadicArguments);
                    }
                    else {
